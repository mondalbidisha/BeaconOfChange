...
function NewsLayout() {
  ...
  const parseGeminiResponse = () => {
    ...
    try {
      geminiGenerate(message, newsDataPrompt)
        .then((response: any) => {
        ...
        <div className="relative min-h-screen w-full overflow-y-auto overflow-x-hidden bg-gradient-to-b from-slate-950 via-indigo-950 to-slate-800 px-20 pb-20 pt-10 scroll-smooth">
          ...
          <FadeIn>
            <div className="flex items-center text-justify mb-10 flex-col text-slate-100 gap-2 text-lg px-20">
              Stay updated on the impact of climate change worldwide and latest developments. Understanding the problem
              is the first step toward recognizing the need for necessary action for a sustainable future.
            </div>
          </FadeIn>
          <FadeInStagger />
          <FadeIn>
